...
def plot_metrics(train_df, test_df, output_dir='plots', file_prefix='cifar100'):
    """
    Generates and saves plots for training and testing metrics with a given prefix.
    """
    ...
    loss_filename = os.path.join(output_dir, f"{file_prefix}_loss_vs_steps.png")
    ...
    plt.title(f'{file_prefix.upper()} Training and Test Loss vs. Steps')
    plt.xlabel('Steps')
    plt.ylabel('Loss')
    plt.legend()
    ...
    accuracy_filename = os.path.join(output_dir, f"{file_prefix}_accuracy_vs_steps.png")
    plt.figure(figsize=(12, 6))
    ...
    plt.title(f'{file_prefix.upper()} Training and Test Accuracy vs. Steps')
    plt.xlabel('Steps')
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.savefig(accuracy_filename)
    plt.close()
    print(f"Saved accuracy plot to {accuracy_filename}")

def main():
    parser = argparse.ArgumentParser(description="Analyze a CIFAR-100 training log file and generate plots.")
    parser.add_argument('log_file', type=str, help="Path to the log file (e.g., cifar100.txt)")
    parser.add_argument('--output_dir', type=str, default='training_plots', help="Directory to save the plots.")
    parser.add_argument('--prefix', type=str, default='cifar100', help="Prefix for the output plot filenames.")
    ...
    plot_metrics(train_df, test_df, args.output_dir, file_prefix=args.prefix)
